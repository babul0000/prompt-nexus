...
export default function Page() {
  ...
  const getToolBadgeStyles = (aiTool = "") => {
    ...
    if (tool.includes("chatgpt")) {
      return "bg-purple-950/40 text-purple-400 border border-purple-500/25";
    }
    if (tool.includes("claude")) {
      return "bg-indigo-950/40 text-indigo-400 border border-indigo-500/25";
    }
    ...
    return "bg-zinc-950/40 text-zinc-400 border border-zinc-500/25";
  };
  ...
  return (
    <div className="min-h-screen bg-[#030014] text-white p-4 sm:p-6 md:p-8 relative">
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-6">
        ...
        {reviews.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 bg-white/[0.02] border border-white/5 rounded-2xl text-center space-y-4 max-w-xl mx-auto">
            <p className="text-sm text-zinc-400 font-medium">You haven't posted any reviews yet.</p>
          </div>
        ) : (
              ...
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.01]">
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">Prompt Title</th>
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">AI Tool</th>
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">Rating</th>
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">Comments</th>
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">Submitted Date</th>
                    <th className="p-4 text-xs font-bold uppercase tracking-wider text-zinc-400">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {reviews.map((rev) => (
                    <tr key={rev._id || rev.id} className="hover:bg-white/[0.01] transition-colors">
                      <td className="p-4 text-sm font-bold text-white max-w-[250px] truncate">
                      ...
                      <td className="p-4 text-sm text-zinc-300 max-w-[200px] truncate" title={rev.comment}>
                        "{rev.comment}"
                      </td>
                      ...
                      <td className="p-4">
                        <Link
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-[#131735]/40 hover:bg-[#131735]/60 border border-[#1e2554] transition-all rounded-lg cursor-pointer"
                        >
