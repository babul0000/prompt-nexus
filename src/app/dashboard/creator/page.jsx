    ...
    return (
        <div className="relative min-h-screen bg-transparent text-zinc-900 dark:text-white pt-10 pb-20 px-4 sm:px-6 transition-colors duration-300">
            ...
            <div className="max-w-6xl mx-auto relative z-10 space-y-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 border-b border-zinc-200 dark:border-white/5 pb-8">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white leading-tight">
                            Welcome Back, <span className="bg-gradient-to-r from-[#7C3AED] to-[#9333EA] dark:from-purple-400 dark:to-pink-400 bg-clip-text text-transparent">{user?.name || 'Creator'}</span>! ✨
                        </h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <div className="bg-white dark:bg-[#090a16]/60 border border-zinc-200 dark:border-white/[0.06] hover:border-amber-500/30 p-6 rounded-2xl transition-all flex items-center justify-between shadow-sm dark:shadow-none text-zinc-900 dark:text-white">
                        <div className="space-y-1">
                            ...
                            <h3 className="text-2xl font-black text-zinc-900 dark:text-white">{totalBookmarks}</h3>
                        </div>
                    ...
                    <div className="bg-white dark:bg-[#090a16]/60 border border-zinc-200 dark:border-white/[0.06] hover:border-emerald-500/30 p-6 rounded-2xl transition-all flex items-center justify-between shadow-sm dark:shadow-none text-zinc-900 dark:text-white">
                        <div className="space-y-1">
                ...
                <div className="bg-white dark:bg-[#090a16]/50 border border-zinc-200 dark:border-white/[0.06] p-6 rounded-2xl flex flex-col gap-6 backdrop-blur-md shadow-sm dark:shadow-none text-zinc-900 dark:text-white">
                    <div className="flex items-center justify-between border-b border-zinc-200 dark:border-white/5 pb-4">
                        <div className="flex items-center gap-2">
                            <LayoutGrid className="w-4 h-4 text-purple-650 dark:text-purple-400" />
                    ...
                    {prompts.length === 0 ? (
                        <div className="flex flex-col items-center justify-center p-12 text-center space-y-4">
                            <Sparkles className="w-10 h-10 text-zinc-400 dark:text-zinc-600 animate-bounce" />
                            <div className="space-y-1">
                                <h4 className="text-sm font-semibold text-zinc-900 dark:text-white">No Prompts Added</h4>
                        ...
                        <div className="overflow-x-auto w-full">
                            <table className="w-full border-collapse text-left text-sm text-zinc-700 dark:text-zinc-300">
                                <thead>
                                    <tr className="border-b border-zinc-200 dark:border-white/5 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                                ...
                                <tbody className="divide-y divide-zinc-150 dark:divide-white/5">
                                    {prompts.slice(0, 5).map((p) => (
                                        <tr key={p._id} className="hover:bg-zinc-50 dark:hover:bg-white/[0.01] transition-colors">
                                            ...
                                            <td className="py-4 px-4">
                                                <span className="bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-300">
                                                    {p.aiTool || 'Other'}
                                                </span>
                                            </td>
                                            <td className="py-4 px-4">
                                                <span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${p.status?.toLowerCase() === 'approved' ? 'bg-emerald-500/10 text-emerald-650 dark:text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20'}`}>
                                                    <span className={`w-1 h-1 rounded-full ${p.status?.toLowerCase() === 'approved' ? 'bg-emerald-650 dark:bg-emerald-500' : 'bg-amber-500'}`} />
                                                    {p.status || 'PENDING'}
                                                </span>
                                            </td>
                                            <td className="py-4 px-4 font-semibold text-zinc-900 dark:text-white">{p.copyCount || 0}</td>
                                            <td className="py-4 pl-4 text-right text-zinc-500 dark:text-zinc-400 font-medium capitalize">{p.visibility || 'Public'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
