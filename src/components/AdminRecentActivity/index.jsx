...
export default function AdminRecentActivity({ activities }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-[#0a0d26]/50 border border-[#13193e] p-5 rounded-2xl flex flex-col gap-4 relative overflow-hidden min-h-[300px]">
        ...
        <div className="flex flex-col gap-3 overflow-y-auto max-h-[220px] scrollbar-thin">
          {recentUsers.length === 0 ? (
            ...
            recentUsers.map(u => (
              <div key={u.id} className="flex items-center justify-between text-xs p-2.5 bg-white/[0.01] border border-white/5 rounded-xl hover:bg-white/[0.02] transition">
                <div className="min-w-0 text-left">
                  <div className="font-bold text-white truncate max-w-[150px]">{u.name}</div>
                  <div className="text-[10px] text-zinc-400 truncate max-w-[150px]">{u.email}</div>
                </div>
      ...
      <div className="bg-[#0a0d26]/50 border border-[#13193e] p-5 rounded-2xl flex flex-col gap-4 relative overflow-hidden min-h-[300px]">
        ...
        <div className="flex flex-col gap-3 overflow-y-auto max-h-[220px] scrollbar-thin">
          {recentPrompts.length === 0 ? (
            ...
            recentPrompts.map(p => (
              <div key={p.id} className="flex items-center justify-between text-xs p-2.5 bg-white/[0.01] border border-white/5 rounded-xl hover:bg-white/[0.02] transition">
                <div className="min-w-0 text-left">
                  <div className="font-bold text-white truncate max-w-[140px]">{p.title}</div>
                  <div className="text-[9px] text-blue-400 uppercase font-semibold mt-0.5">{p.category || "General"}</div>
                </div>
                <div className="flex items-center gap-1 text-[10px] text-zinc-500 shrink-0">
                  <Calendar className="w-3 h-3" />
        ...
        <div className="flex flex-col gap-3 overflow-y-auto max-h-[220px] scrollbar-thin">
          {recentPayments.length === 0 ? (
            ...
            recentPayments.map(pay => (
              <div key={pay.id} className="flex items-center justify-between text-xs p-2.5 bg-white/[0.01] border border-white/5 rounded-xl hover:bg-white/[0.02] transition">
                <div className="min-w-0 text-left">
                  <div className="font-bold text-white truncate max-w-[130px]">{pay.name}</div>
                  <div className="text-[9px] text-emerald-400 font-semibold mt-0.5">{pay.email}</div>
                </div>
                <div className="flex flex-col items-end shrink-0 gap-1">
                  <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">+$5.00</span>
